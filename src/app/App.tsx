...
import { Mic, Activity, Terminal, Sparkles, BarChart3, Settings as SettingsIcon } from 'lucide-react';
import { Tabs, TabsContent, TabsList, TabsTrigger } from './components/ui/tabs';

export default function App() {
  const [isListening, setIsListening] = useState(false);
  const [systemStatus, setSystemStatus] = useState<'online' | 'offline' | 'booting'>('booting');

  useEffect(() => {
    // Simulate system boot
    const timer = setTimeout(() => setSystemStatus('online'), 2000);
    ...
    <div className="dark min-h-screen bg-gradient-to-br from-slate-950 via-blue-950 to-slate-900 text-white">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              <div className={`px-4 py-2 rounded-full text-sm font-medium ${
                ...
                systemStatus === 'booting' ? 'bg-yellow-500/20 text-yellow-400' :
                'bg-red-500/20 text-red-400'
              }`}>
                {systemStatus === 'online' ? '● ONLINE' : 
                 systemStatus === 'booting' ? '◐ BOOTING' : '○ OFFLINE'}
              </div>
            </div>
          </div>
        </div>
      ...
      <main className="container mx-auto px-6 py-8">
        <Tabs defaultValue="dashboard" className="space-y-6">
          <TabsList className="grid grid-cols-6 w-full max-w-3xl mx-auto bg-slate-900/50 border border-blue-500/20">
            ...
            <TabsTrigger value="control" className="data-[state=active]:bg-blue-500/20">
              <Terminal className="w-4 h-4 mr-2" />
              Control
            </TabsTrigger>
            <TabsTrigger value="showcase" className="data-[state=active]:bg-blue-500/20">
          ...
          <TabsContent value="dashboard">
            <Dashboard isListening={isListening} setIsListening={setIsListening} />
          </TabsContent>
          ...
          <TabsContent value="voice">
            <VoiceInterface isListening={isListening} setIsListening={setIsListening} />
          </TabsContent>
      ...
      <footer className="border-t border-blue-500/20 bg-slate-950/50 backdrop-blur-xl mt-16">
        <div className="container mx-auto px-6 py-4 text-center text-sm text-slate-400">
          <p>JARVIS Ultimate System v1.0.0 | Built with React + Tailwind CSS</p>
